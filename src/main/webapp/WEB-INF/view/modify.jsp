	...
	<div>
		<div id="navbar" class="navbar-collapse collapse">
			<ul class="nav navbar-nav">
				<li><a href="checkLoginInAction.do?inputUser=<%=session.getAttribute("inputUser")%>&inputPassword=<%=session.getAttribute("inputPassword")%>" style="font-weight: bold;">Inserisci
						Nuova Inserzione</a></li>
				<li class="active"><a href="modify.do" style="font-weight: bold;">Modifica
						Inserzione</a></li>
			</ul>
		</div>
		<!--/.nav-collapse -->
			...
			<div class="col-lg-4">
				<div class="panel panel-default" id="headings">
					...
					<div class="panel-body">
						<form id="modifyAd" name="modifyAd" action="modifyAd.do"
							method="POST">
							<div class="form-group" style="display: none;">
								<input class="form-control" type="text" id="idFlat" name="idFlat">
							</div>
							<div class="form-group">
								<label for="catFLat">Categoria</label> <select
									class="form-control" id="catFLat" name="catFLat">
									<option id="0" value="Tutte">Tutte</option>
									<option id="1" value="Residenziale">Residenziale</option>
									<option id="2" value="Commerciale">Commerciale</option>
									<option id="3" value="Imperdibili">Imperdibili</option>
								</select>
							</div>
							<div class="form-group">
								<label for="tipFLat">Tipologia</label> <select
									...
									<option id="0" value="Tutte">Tutte</option>
									<option id="1" value="Appartamenti">Appartamenti</option>
									<option id="2" value="Ville">Ville</option>
									<option id="3" value="Rustici/Cascine/Case">Rustici/Cascine/Case</option>
									<option id="4" value="Box/posti auto">Box/posti auto</option>
									<option id="5" value="Terreno">Terreno</option>
								</select>
							</div>
							<div class="form-group">
							...
							<button class="btn btn-lg btn-primary btn-block" type="submit"
								onclick="return validation()">Modifica ></button>
						</form>
					</div>
				</div>
			</div>
			<div class="col-lg-8">
				<div class="panel panel-default" id="headings">
					<div class="col-lg-3">
							...
							<h5 style="font-weight: bold;">Ricerca per ID</h5>
							<!-- <div class="panel panel-default" id="headings"> -->
							<div class="table-responsive">
							<form class="form-signin" action="getInfoFlatByIdModify.do"
							method="post">
							<!-- <h2 class="form-signin-heading">Entra come amministratore</h2> -->
							<!-- <label for="inputUser" class="sr-only">Email address</label>  -->
								<input id="idFlat" name="idFlat" class="form-control"
									placeholder="Id Flat" required autofocus> 
						...
						<div class="bs-docs-header" id="content" tabindex="-1">
							...
							<div class="table-responsive">
								<table class="table table-striped table-bordered">
									<tbody>
										<c:forEach items="${allFlats}" var="informazioniFlat">
											<c:forEach items="${informazioniFlat.foto}" var="fotoFlat">
												<c:set var="fotoName" value="${fotoFlat.pathFoto}" />
											</c:forEach>
											<tr>

												<td align="center"><button   id="pk_${informazioniFlat.id}" onclick="getInfoFlatModify(${informazioniFlat.id})">${informazioniFlat.id}</button></td>
...
<script>
	//With JQuery
	$(function() {
		$('#modifyAd').find(':input:not(:disabled)').prop('disabled',true);
	...
	/* I need to elaborate data without submit different from that above */
	function getInfoFlatModify(idFlat)
	{
		$.ajax({
			  type: "POST",
			  url: "getInfoFlatModify.do",
			  data: {idFlat: idFlat},
			  dataType : "json",
			  async:false,
			  success: function (data) {
				  $('#modifyAd').find(':input:disabled').prop('disabled',false);
				  
				  for(var item in data.listaComuni) {
					  $('#comFLat').append('<option id="' + data.listaComuni[item].idProvinciaFlat + ' value="' + data.listaComuni[item].idProvinciaFlat + '">' + data.listaComuni[item].nomeProvinciaFlat + '</option>');
				  } 
				  /*SET COMUNI FROM PROVINCIA  */
				  
				  $("#idFlat").val(data.id);
				  $("#catFLat").val(data.categoria);
				  $("#tipFLat").val(data.tipologia);
				  $("#provFlat").val(data.provincia);
				  $('#comuneCheck').show();
				  $("#comFLat").val(data.comune);
				  $("#przA").slider({});
				  $("#przV").slider({});
				  if(data.affittasi == true)
				  {
					  $('#vendesiCheck').hide();
					  $('#fittasiCheck').show();
					  $("#przA").data('slider').setValue(data.prezzo);
				  }
				  if(data.affittasi == false)
				  {
					  $('#fittasiCheck').hide();
					  $('#vendesiCheck').show();
					  $("#przV").data('slider').setValue(data.prezzo);
				  }
				  $("#supFlat").slider({});
				  $("#supFlat").data('slider').setValue(data.superficie);
				  $("#fitt").prop('checked',data.affittasi);
				  $("#vend").prop('checked',data.vendesi);
				  $("#numLocFlat").slider({});
				  $("#numLocFlat").data('slider').setValue(data.locali);
				  $("#numBagFlat").slider({});
				  $("#numBagFlat").data('slider').setValue(data.bagni);
				  
				  $("#box").prop('checked',data.boxAuto);
				  $("#giar").prop('checked',data.giardino);
				  $("#bal").prop('checked',data.balcone);
				  $("#terr").prop('checked',data.terrazzo);
				  $("#asc").prop('checked',data.ascensore);
				  $("#descrizioneFlat").val(data.descrizione);
